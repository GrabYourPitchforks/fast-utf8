...
namespace FastUtf8Tester
{
    ...
    public struct Utf8ValidityChecker
    {
        ...
        private unsafe static uint ConsumeDataWithExistingPartialSequence(uint partialSequence, ReadOnlySpan<byte> utf8Bytes)
        {
            ...
            Span<byte> partialSequenceAsBytes = new Span<byte>(&partialSequence, sizeof(uint));
        ...
        private static unsafe uint ConsumeDataWithoutExistingPartialSequence(ReadOnlySpan<byte> utf8Bytes)
        {
            ...
            else
            {
                ...
                if (validity == SequenceValidity.Incomplete)
                {
                    ...
                    if (BitConverter.IsLittleEndian)
                    {
                        uint partialSequence = (uint)numBytesConsumed;
                        utf8Bytes.Slice(0, numBytesConsumed).CopyTo(new Span<byte>(&partialSequence, sizeof(uint)).Slice(1));
                        return partialSequence;
                    }
                    else
                    {
                        uint partialSequence = (uint)numBytesConsumed;
                        utf8Bytes.Slice(0, numBytesConsumed).CopyTo(new Span<byte>(&partialSequence, sizeof(uint)));
                        return partialSequence;
                    }
                }
                else
